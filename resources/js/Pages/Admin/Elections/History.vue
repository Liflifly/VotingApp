<template>
  <AuthenticatedLayout title="RIWAYAT PERIODE">
    <!-- Header -->
    ...
    <div v-if="!elections.length" class="neo-card p-8 md:p-12 text-center relative overflow-hidden">
      ...
      <p class="font-body text-sm md:text-body-md text-neo-grey">Belum ada periode yang telah berakhir.</p>
    </div>
    <div v-else class="space-y-3 md:space-y-4">
      <div v-for="e in elections" :key="e.id" class="neo-card p-4 md:p-5 relative overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          <div>
            <h3 class="font-heading font-bold text-xs md:text-sm uppercase">{{ e.name }}</h3>
